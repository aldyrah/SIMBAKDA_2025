...
// fungction baru aldi FORM KIB_A
        ...
        function mapl44()
    {
        ...
        $data['page_title']= 'INPUT MAP L44';
        $this->template->set('title', 'INPUT MAP L44');   
        $this->template->load('index','master/mapl44',$data) ;
        } 
    }

function load_mapl44() {
        if($this->auth->is_logged_in() == false){
                redirect(site_url().'/welcome/login');
           }else{
         $page = isset($_POST['page']) ? intval($_POST['page']) : 1;
         ...
         $where2='';
         if($kriteria <> ''){
           $where2 =" where UPPER(a.nama) and a.unit_skpd = '$unit_skpd' LIKE UPPER('%$kriteria%')";
         }
                 
          
         $rs = $this->db->query("select count(*) as tot FROM mrekap_format_ivl44 where  unit_skpd = '$unit_skpd'    order by no_urut");
         $trh = $rs->row();
         //$result["total"] = $trh->tot;
         
         $sql = "SELECT * FROM mrekap_format_ivl44 a  where  unit_skpd = '$unit_skpd' order by no_urut limit $offset,$rows";
         $query1 = $this->db->query($sql);  
         //$result = array();
         ...
         { 
             $coba[] = array(
                         ...
                         'seq' => $resulte['seq'],
                         'nama' => $resulte['nama'],
                         'nilai_ini' => $resulte['nilai_ini'],
                         'jml_brg' => $resulte['jml_brg'],
                         'unit_skpd' => $resulte['unit_skpd']
                         );
                         $ii++;
         }
