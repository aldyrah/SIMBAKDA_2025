...
class Utilitas extends CI_Controller {
	...
	function backup(){
		...
		foreach($query->result_array() as $row){
			$nm=$row['Tables_in_simakda_siadinda_biak'];
	...
	function do_upload(){
        ...
        //http://localhost/simakda_baru/ 
		$fl= $this->input->post('datasql');
		$upload_path_url = base_url().'upload/';
        $config['upload_path'] = './upload/';
        $config['allowed_types'] = 'jpg|jpeg|txt|dat|sql|abc';
	...
	function proses_ahirtahun(){	
		...
		foreach($query->result_array() as $row){
			$nm=$row['Tables_in_simakda_siadinda_biak'];
			$query = $this->db->query(" CREATE TABLE $nama.$nm LIKE simakda_siadinda_biak.$nm ");  			
			...
			if((strtoupper(substr($nm,0,1))=='M') or (in_array($nm,$copytabel) )){
				$query = $this->db->query(" insert into $nama.$nm select * from simakda_siadinda_biak.$nm ");  					
			}
		}
