...
<script type="text/javascript">
    ...
    //susut();
      ...
      /*$('#pelihara').flatcheckbox({
        label: "Kapitalisasi KIB (522,523) / Pemeliharaan",
        onChecked: function () {
          //checked(el);
          susut();
          sts_inp=1;
    ...
    /*function checked(el) {
    var id = $(el).attr('id');
    //$('#prompt').html('Kapitalisasi KIB (522,523)');
  }
  function unChecked(el) {
    var id = $(el).attr('id');
    //$('#prompt').html('Pemeliharaan');
  }*/
      ...
      url: '<?php echo base_url(); ?>/index.php/master/ambil_msskpd2',  
       ...
       onSelect:function(rowIndex,rowData){
          // if (lcstatus == 'tambah'){
           ...
           $("#nmunit").attr("value",rowData.nm_lokasi);
           ...
           if(lcstatus=='tambah'){
            ...
            $('#no').attr('value',''); 
            $('#tgl_reg').attr('value','');
            $('#kd_golongan').attr('value',''); 
            $('#luas_gedung').attr('value',''); 
            $('#jenis_gedung').attr('value',''); 
     ...
     $('#dg').edatagrid({
        ...
        onDblClickRow:function(rowIndex,rowData){
          ...
          luas_gedung     = rowData.luas_gedung;
          jenis_gedung    = rowData.jenis_gedung;
          luas_tanah      = rowData.luas_tanah;
		  ...
		  get1(no_bukti,kd_skpd,nm_skpd,kd_unit,nm_unit,tgl_kap,kd_brg,nm_brg,no_dokumen,tgl_perolehan,hrg_perolehan,nilai,masa_manfaat,persen,tmbh_manfaat,ket_kap,id_barang,no_reg,no,tgl_reg,luas_gedung,jenis_gedung,luas_tanah,status_tanah,kondisi,keterangan,tahun,alamat1,total_oleh_kap,bidang,nm_bidang,jns_trans);  
      load_detail(no_bukti,kd_skpd);
...
function load_detail(no_bukti,kd_skpd){
  ...
  $(function(){
            $('#dg_detail').edatagrid({
                url: '<?php echo base_url(); ?>/index.php/transaksi/load_dkap_c',
...
$('#bidang').combogrid({  
            ...
            columns:[[  
             ...
             onSelect:function(rowIndex,rowData){
                ...
                if(lcstatus =='tambah'){
                  ...
                  $('#kib').combogrid({url:'<?php echo base_url(); ?>index.php/transaksi/kib_c_kap',
	   ...
	   $('#kib').combogrid({  
            ...
            columns:[[  
               ...
               {field:'luas_tanah',title:'Luas Tanah',width:70,align:"right"},  
           ...
           onSelect:function(rowIndex,rowData){
                        ...
                        $('#kd_golongan').attr('value',rowData.kd_golongan);
  ...
  function susut(){
    ...
    $(document).ready(function(){
            ...
            $.ajax({
                ...
                data: ({id_b:id_b,tahun:tahun,table:'trkib_c'}),
                dataType:"json",
    ...
    function get1(no_bukti,kd_skpd,nm_skpd,kd_unit,nm_unit,tgl_kap,kd_brg,nm_brg,no_dokumen,tgl_perolehan,hrg_perolehan,nilai,masa_manfaat,persen,tmbh_manfaat,ket_kap,id_barang,no_reg,no,tgl_reg,luas_gedung,jenis_gedung,luas_tanah,status_tanah,kondisi,keterangan,tahun,alamat1,total_oleh_kap,bidang,nm_bidang,jns_trans){
            ...
            $('#kd_golongan').attr('value',''); 
            $('#luas_gedung').attr('value',luas_gedung); 
            $('#jenis_gedung').attr('value',jenis_gedung); 
            $('#luas_tanah').attr('value',luas_tanah); 
            $('#status_tanah').attr('value',status_tanah); 
            $('#kd_tanah').attr('value',''); 
            $('#kondisi').attr('value',kondisi);
            $('#keterangan').attr('value',keterangan);  
            $('#tahun').attr('value',tahun); 
            $('#alamat').attr('value',alamat1);
...
//           alert(test);
    ...
    function kosong(){
        ...
        $('#skpd').combogrid('enable');
           ...
           $('#tambah').linkbutton('enable');
           $('#hapus').linkbutton('enable');
           //$('#pelihara').flatcheckbox('label','Pemeliharaan / Kapitalisasi KIB (522,523)');
        set_grid();
    ...
    function simpan(){
            ...
            var kd_golongan = document.getElementById('kd_golongan').value; 
            var luas_gedung = document.getElementById('luas_gedung').value; 
            var jenis_gedung= document.getElementById('jenis_gedung').value; 
		...
		//lcvalues = "('"+no_reg+"','"+id_barang+"','"+no+"','"+tgl_reg+"','"+no_dokumen+"','"+kd_brg+"','"+nilai+"','"+jumlah+"','"+total+"','"+merek+"','"+tipe+"','"+pabrik+"','"+kd_warna+"','"+kd_bahan+"','"+kd_satuan+"','"+no_rangka+"','"+no_mesin+"','"+no_polisi+"','"+silinder+"','"+no_stnk+"','"+tgl_stnk+"','"+no_bpkb+"','"+tgl_bpkb+"','"+kondisi+"','"+tahun_produksi+"','"+nip+"','"+dasar+"','"+no_sk+"','"+tgl_sk+"','"+keterangan+"','"+kd_ruang+"','"+kd_lokasi2+"','"+kd_unit+"','"+tahun+"','"+tgl_sp2d+"','"+foto+"','"+tgl_perolehan+"','"+peroleh+"','"+tmbh_manfaat+"')";
			...
			$(document).ready(function(){
                $.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>index.php/transaksi/simpan_trkib_c_kap',
                    data: ({tabel:'trkib_c_kap',
    ...
    function simpan_trdkapb_dh(){
        ...
        for(var p=0;p<rows.length;p++){
              ...
              xrek           = rows[p].kd_rek5;
                ...
                $.ajax({
                    ...
                    dataType:'json', 
                    url: '<?php echo base_url(); ?>/index.php/transaksi/simpan_trkib_c_kap',   
                    data: ({tabel:'trdkibc_kap',bukti:nomor_bukti,skpd:skpd,unit:unit,id_b:id_b,sql:csql,gab:gab,jns_trans:jns_trans}),
     ...
     function hapus(){
      ...
      if(jns_trans=='1' && sts_reklas=='SR'){
		...
		if (del==true){     
        var urll = '<?php echo base_url(); ?>index.php/transaksi/hapus_trkib_c_kap_dh';
        $(document).ready(function(){
...
//alert(frek);
    ...
    $('#nodok').combogrid({  
       ...
       mode:'remote',
       loadMsg:"Tunggu Sebentar....!!",
       url:'<?php echo base_url(); ?>index.php/master/ambil_nomor_kontrak_kap_c',
...
<h3><a href="#" id="section1"><i>LIST KAPITALISASI KIB C</i></a></h3>
    <div>
      <p align="left">
    ...
    <p align="center">
      <table align="center" id="dg" title="LIST KAPITALISASI KIB C" style="width:940px;height:350px;" >  
        </table>
    </p>
    </div> 

<h3><a href="#" id="section2"><i>INPUT KAPITALISASI KIB C</i></a></h3>
<div >
     ...
     <table align="center" style="width:100%;" border="0">
           <tr>
             ...
             <tr hidden="true">
                       ...
                       <tr hidden="true">
                            <td>kd_golongan</td>
                            <td>: <input id="kd_golongan" /></td>
                            <td></td>
                       </tr>
                       <tr hidden="true">
                            <td>luas_gedung</td>
                            <td>: <input id="luas_gedung" /></td>
                            <td></td>
                       </tr>
                       <tr hidden="true">
                            <td>jenis_gedung</td>
                            <td>: <input id="jenis_gedung" /></td>
                            <td></td>
                       </tr>
                       <tr hidden="true">
                            <td>luas_tanah</td>
                            <td>: <input id="luas_tanah" /></td>
                            <td></td>
                       </tr>
                       <tr hidden="true">
                            <td>status_tanah</td>
                            <td>: <input id="status_tanah" /></td>
                            <td></td>
                       </tr>
                       <tr hidden="true">
                            <td>kd_tanah</td>
                            <td>: <input id="kd_tanah" /></td>
                            <td></td>
                       </tr>
