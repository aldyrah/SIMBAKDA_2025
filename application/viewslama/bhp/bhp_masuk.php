    ...
    $(function(){         
         $('#trh').edatagrid({
            ...
            onSelect:function(rowIndex,rowData){ 
                ...
                nspj        =rowData.nspj;
                nspj2       =rowData.nspj2;
                get(nomor,tgl,unit,kode,nmkode,tahun,total,nm_kegiatan,nm_perush,no_terima,tgl_terima,kodegiat,kd_rek5,nspj,nspj2);
                load_detail();
...
if (plonline == '1'){
  $('#nomor').combogrid({url:'<?php echo base_url(); ?>index.php/bhp/ambil_nomor_trans',  
            panelWidth:420, 
            width:160, 
            idField:'no_bukti',  
            textField:'no_bukti',              
            ...
            loadMsg:"Tunggu Sebentar....!!",                                                 
            columns:[[  
               {field:'no_bukti',title:'No Bukti',width:130},  
               {field:'tgl_bukti',title:'Tanggal',width:80},
               {field:'kd_skpd',title:'SKPD',width:80},
               {field:'nilai2',title:'Nilai',width:100,align:"right"},               
            ]],  
            onSelect:function(rowIndex,rowData){  
                                                                      
                cnm = rowData.tgl_bukti;
                skpd= rowData.kd_skpd;              
                $('#tanggal').datebox('setValue',cnm);
                $('#uskpd').combogrid('clear');
                $('#uskpd').combogrid('grid').datagrid('reload');
                $('#nmuskpd').attr('value','');
                if (plonline=='1'){
                  $('#kdskpd').combogrid('disable');
                  $('#kdskpd').combogrid('setValue',rowData.kd_skpd);
                  $('#giat').combogrid('setValue',rowData.kd_kegiatan);
                  $('#nmgiat').attr('value',rowData.nm_kegiatan);
                  $('#kd_rek5').attr('value',rowData.kd_rek5);
                  $('#nilai_spj').attr('value',rowData.nilai);
                  $('#nilai_spj2').attr('value',rowData.nilai2);
                    //validate_giat();
                }
            } 
        }); 
	...
	$('#giat').combogrid({
            //url:'<?php echo base_url(); ?>index.php/bhp/ambil_giat',
           //url:'<?php echo base_url(); ?>index.php/bhp/ambil_keg_dh',
...
//demansyah
function validate_kdbrg(){
  var kode5 = document.getElementById('kd_rek5').value; 
  $('#trd2').datagrid('selectAll');
           var rows = $('#trd2').datagrid('getSelections');     
           frek  = '' ;
           rek5  = '' ;
           for ( var p=0; p < rows.length; p++ ) { 
           rek5 = rows[p].kode_brg;                                       
           if ( p > 0 ){   
                  frek = frek+','+rek5;
              } else {
                  frek = rek5;
  ...
  $(function(){
      $('#kd').combogrid({url:'<?php echo base_url(); ?>index.php/bhp/ambil_brg',  
            queryParams:({kdrekbrg:'',kode5:kode5}),
            panelWidth:600, 
            width:160, 
            idField:'kd_brg',  
            textField:'nm_brg',              
  ...
  $('#trd2').datagrid('unselectAll');
    ...
    function get(nomor,tgl,unit,kode,nmkode,tahun,total,nm_kegiatan,nm_perush,no_terima,tgl_terima,kodegiat,kd_rek5,nspj,nspj2){
        if(plonline=='1'){
        ...
        $('#nomor_terima').attr('value',no_terima);
        
        $('#tanggal_terima').datebox('setValue',tgl_terima);
        $('#giat').combogrid('setValue',kodegiat);
        $('#giat').combogrid('disable');
        $('#nmgiat').attr('value',nm_kegiatan);
        $('#nm_perusahaan').attr('value',nm_perush);
        $('#nmuskpd').attr('value',nmkode);
        $('#tahun').combobox('setValue',tahun);
        $('#total').attr('value',total);
        $('#kdskpd').combogrid('disable');
        $('#uskpd').combogrid('disable');
        $('#nomor_terima').attr('disabled',true);
        $('#kd_rek5').attr('value',kd_rek5);
        $('#nilai_spj').attr('value',nspj2);
        $('#nilai_spj2').attr('value',nspj);
...
/* 		$('#no_bast_edit').attr('value',no_dokumen);
    ...
    function kosong2(){
        ...
        $('#kd').combogrid('clear');
        $('#kd').combogrid('grid').datagrid('reload');
        $('#nm').attr('value','');
        $('#nm_giat').attr('value','');
        $('#nm_asal').attr('value','');
        $('#nm_dana').attr('value','');
              ...
              /*   dataType:"json", */ 
                ...
                success:function(data){                                          
                    $.each(data,function(i,n){ 
						...
						//$('#total').attr('value',number_format(jmltot),2,'.',',');		
						$('#dana').combogrid('setValue',sdn);
						$('#asal').combogrid('setValue',asl);
                    $('#trd').edatagrid('appendRow',{idx:pidx,no_dokumen:no,kode_brg:kd,detail_brg:nm,merk:mrk,jumlah:jml,harga:hrg,total:tot,keterangan:ket,kodegiat:kdgiat,satuan:satu,sdana:sdn,asal:asl});                                
    ...
    function load_detail2(){  
       ...
       if (rows.length==0){
		...
		for(var p=0;p<rows.length;p++){
            ...
            sdn    = rows[p].sdana;
            asl    = rows[p].asal;                                                                                                               
            $('#trd2').edatagrid('appendRow',{idx2:pidx,no_dokumen:no,kode_brg:kd,detail_brg:nm,merk:mrk,jumlah:jml,harga:hrg,total:tot,keterangan:ket,kodegiat:kdgiat,satuan:satu,sdana:sdn,asal:asl});            
        }
    ...
    function set_grid2(){
         $('#trd2').edatagrid({                                                                   
              columns:[[
                    {field:'idx2',title:'idx2',width:50,align:'left',hidden:true},
					          {field:'no_dokumen',title:'Nomor Dokumen',width:100,hidden:true},
                    {field:'kode_brg',title:'Kode Barang',width:100,align:"center"},
                    {field:'detail_brg',title:'Nama Barang',width:200,align:"left"},
                    {field:'merk',title:'Merek',width:150,align:"left"},
    ...
    var rows = $('#trd').datagrid('getSelections'); 
      ...
      a12.push(rows[i].asal);
        ...
        dn           =(a11.join('||'));
        asl          =(a12.join('||'));
        //alert(no+'/'+kd+'/'+nm+'/'+mrk+'/'+jml+'/'+hrg+'/'+tot+'/'+ket);
    ...
    function append_save(){
      ...
      var rows  = $('#trd2').datagrid('getSelections') ;
        ...
        if (kd != '' && jml != '' && hrg != ''){        
            if (updt == 'f') {
			$('#trd').edatagrid('appendRow',{idx:pidx,no_dokumen:no,kode_brg:kd,detail_brg:nm,merk:mrk,jumlah:jml,harga:hrg,total:tot,keterangan:ket,kodegiat:giat,satuan:satuan,sdana:dn,asal:asl});
			$('#trd2').edatagrid('appendRow',{idx2:pidx,no_dokumen:no,kode_brg:kd,detail_brg:nm,merk:mrk,jumlah:jml,harga:hrg,total:tot,keterangan:ket,kodegiat:giat,satuan:satuan,sdana:dn,asal:asl});
			a = total + tot;
      //validate_kdbrg(); 
      
            } else {
                $('#trd').edatagrid('updateRow',{idx:pidx,no_dokumen:no,kode_brg:kd,detail_brg:nm,merk:mrk,jumlah:jml,harga:hrg,total:tot,keterangan:ket,kodegiat:giat,satuan:satuan,sdana:dn,asal:asl});
                $('#trd2').edatagrid('updateRow',{idx2:pidx,no_dokumen:no,kode_brg:kd,detail_brg:nm,merk:mrk,jumlah:jml,harga:hrg,total:tot,keterangan:ket,kodegiat:giat,satuan:satuan,sdana:dn,asal:asl});                        
    ...
    function get2(kode_brg,detail_brg,merk,jumlah,harga,total,keterangan,kodegiat,satuan,sdana,asal){
     ...
     function simpan(){
        ...
        var nspj        = angka(document.getElementById('nilai_spj2').value);
        ...
        $(document).ready(function(){
            $.ajax({
                type: "POST",       
                dataType : 'json',         
                data: ({tabel:tbl,no:cno,tgl:ctgl,giat:giat,unit:cuskpd,skpd:ckdskpd,no_terima:cno_terima,tgl_terima:ctgl_terima,kdskpd:ckdskpd,nmuskpd:cnmuskpd,tahun:cthn,total:ctotal,comp:cnmcomp,nmgiat:nmgiat,kd_rek5:kd_rek5,nspj:nspj}),
   ...
   function hapus_detail(){
    ...
    function hitung_dh(){
      var harga = angka(document.getElementById('hrg').value);
      var tot2  = angka(document.getElementById('total2').value);
      var spj   = angka(document.getElementById('nilai_spj').value);
      var spj2 = number_format(spj,2,',','.');
      selisih = spj-(harga+tot2);
      if(selisih<0){
        alert('Nilai Melebihi SPJ     '+spj2);
    ...
    <div id="tabs-2">  
        ...
        <table>
            ...
            <tr>   
				      <td>Kegiatan</td>
                <td>:</td>
                <td><input id="giat" name="giat" value=""/> <input type="hidden" id="kd_rek5" name="kd_rek5" value=""/>  </td> 
                <td></td>				
      ...
      <tr>
        <td>Nama Kegiatan</td>
        <td>:</td>
        <td><input type="text" id="nmgiat" style="border:0;width: 400px;" readonly="true"/>  </td> 
      </tr>                      
        </table> 
        ...
        <!-- <fieldset>
        <div align="center">
    		<a class="easyui-linkbutton" iconCls="icon-add" plain="false" onclick="javascript:tambah_detail();"><b>Tambah Barang</b></a> <br/>  
      ...
      <div id="toolbar"  align="center">
        <a class="easyui-linkbutton" iconCls="icon-add" plain="false" onclick="javascript:tambah_detail();"><b>Tambah Barang</b></a>   
        ...
        <table>      
            <tr>
                ...
                <td rowspan="9" width="660"  >
                    ...
                    <div align="left">Jumlah Nilai SPJ : <input type="hidden" id="nilai_spj" style="text-align: right;border:0;width: 200px;font-size: large;" readonly="true"/><input type="text" id="nilai_spj2" style="text-align: right;border:0;width: 200px;font-size: large;" readonly="true"/>
                     &nbsp;&nbsp; Jumlah : <input type="text" id="total2" style="text-align: right;border:0;width: 200px;font-size: large;" readonly="true"/></div>     
                </td>         
            </tr>  
			<tr>
                <td>Perolehan</td>
            ...
            <tr>
                <td>Jumlah</td>
                <td>:</td>
                <td><input id="jml" name="jml" value="" style="text-align: right;" onkeyup="hitung();hitung_dh();"/></td>            
            </tr>  
            <tr>
                <td>Harga</td>
                <td>:</td>
                <td><input id="hrg" name="hrg" value="" style="text-align: right;" onkeypress="return(currencyFormat(this,',','.',event));" onkeyup="hitung();hitung_dh();"/>  </td>            
            </tr> 
            <tr>
                <td>Total Harga</td>
                <td>:</td>
