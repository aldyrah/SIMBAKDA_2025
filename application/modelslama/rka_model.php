...
class Rka_model extends CI_Model {
	...
	{
		$this->db->select('trskpd.kd_urusan,trskpd.kd_skpd,trskpd.kd_kegiatan as giat,m_giat.nm_kegiatan,trskpd.total,trskpd.total_ubah');
		$this->db->from('trskpd');
	...
	//demansyah 7-9-2015
    function getAlldh($limit, $offset,$id)
    {
        $this->db->select('trskpd.kd_urusan,trskpd.kd_skpd,trskpd.kd_kegiatan as giat,m_giat.nm_kegiatan');
        $this->db->select('SUM(trdrka.nilai)as nilai,SUM(trdrka.nilai_ubah)as nilai_ubah');
        $this->db->from('trskpd');
        $this->db->join('m_giat','m_giat.kd_kegiatan=trskpd.kd_kegiatan1');
        $this->db->join('trdrka','trskpd.kd_kegiatan=trdrka.kd_kegiatan');
        $this->db->where('trskpd.kd_skpd', $id);
        $this->db->where('trskpd.jns_kegiatan','52');
        $this->db->order_by('trskpd.kd_skpd');
        $this->db->group_by('trdrka.kd_kegiatan');
        $this->db->order_by('trskpd.kd_kegiatan', 'asc');
        $this->db->limit($limit,$offset);
        return $this->db->get();
    }
    ...
    function combo_beban($id='',$script=''){
        ...
        $cRet   .= "<option value='62'>Pengeluaran Pembiayaan</option>";        
        $cRet   .= "</select>";        
        return $cRet;
    }
...
//        }
	...
	function _mpdf3($judul='',$isi='',$lMargin='',$rMargin='',$font=0,$orientasi='L',$kertas='A4',$tmargin='') {
		...
		//$this->mpdf->SetFont($family,$style='',$size=0, $write=true, $forcewrite=false);
    ...
    function set_history($username,$update,$aksi,$nomor,$setuju,$uraian){
        $query = 'INSERT INTO history(username,tgl_update,aksi,nomor,persetujuan,ket)values("'.$username.'","'.$update.'","'.$aksi.'","'.$nomor.'","'.$setuju.'","'.$uraian.'")';
        $this->db->query($query);
               
    }

	
	function getAll_lunch_dh($limit, $offset,$id)
    {
        $this->db->select('trskpd.kd_urusan,trskpd.kd_skpd,trskpd.kd_kegiatan as giat,m_giat.nm_kegiatan');
        $this->db->select('SUM(trdrka.nilai)as nilai,SUM(trdrka.nilai_ubah)as nilai_ubah');
        $this->db->from('trskpd');
        ...
        $this->db->where('trskpd.kd_skpd', $id);
        $this->db->where('trskpd.jns_kegiatan','52');
        $this->db->where('trskpd.lanjut','Ya');
        $this->db->order_by('trskpd.kd_skpd');
        $this->db->group_by('trdrka.kd_kegiatan');
        $this->db->order_by('trskpd.kd_kegiatan', 'asc');
        $this->db->limit($limit,$offset);
