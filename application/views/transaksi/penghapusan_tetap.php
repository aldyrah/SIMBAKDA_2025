...
<script type="text/javascript">
    ...
    $(function(){
         ...
         url: "<?php echo base_url(); ?>index.php/transaksi/ambil_hapus_head",
			...
			pagination:"true",                      
            columns:[[
                {field:'no_bukti',title:'bukti',width:100,hidden:true},
                {field:'sts',title:'Status',width:80},
				...
				{field:'no_hapus',title:'NO B.A HAPUS',width:150},
        	    {field:'tgl_hapus',title:'TGL B.A HAPUS',width:100},
        	    {field:'baru',title:'OPD',width:250},
        ...
        onDblClickRow:function(rowIndex,rowData){  
			...
			if(tanggal==''){
				...
				if(sts=='DISETUJUI' || sts=='DITOLAK'){
					sweetAlert("Tidak Dapat Di Tetapkan Ulang", "Data Telah Di Lakukan Penetepan", "error");
					...
					$('alasan').attr('value',alasan);
		            $("#dialog-modal").dialog('open'); 
		            $('#tetap').linkbutton("enable");
					loadDetail();
				}
			}		
		}
...
function set_grid(){
    $('#trd_hapus').edatagrid({
columns:[[  {field:'no_reg',title:'NO REG',width:50,hidden:true},
    ...
    function simpan(){
      ...
      $('#trd_hapus').datagrid('selectAll');
		...
		var rwyt_baru 	= ("Hasil PENGHAPUSAN dari OPD "+skpd_lama);
    ...
    if(r==true){
    	...
    	$("#dialog-modal-aaa").dialog('open');
         $(document).ready(function(){
			...
			$('#trd_hapus').edatagrid('unselectAll');
			$('#tetap').linkbutton("enable");
			$("#dialog-modal-aaa").dialog('close');
		}
     }
   ...
   function keluar(){
	...
	function tolak(){
        ...
        $(document).ready(function(){
         $.post(urll,({no_hapus:no_hapus,skpdb:skpd_baru,alasan:alasan,bukti:bukti,tgl_mut:ctgl_muts}),
		 function(data){
					swal({
					title: "",
...
<p><h3 align="center">PENETAPAN PENGHAPUSAN BARANG</h3></p>
    <div id="isian">  
		...
		<tr>
			...
			<td><a plain="false">Cari</a>
                <input id="txtcari" name="txtcari" class="easyui-searchbox" 
                data-options="prompt:'Please Input Value',searcher:function(value,name){cari2(value)}" 
                style="width:180px"/>
            </td>				
        ...
        <table  id="trd" style="width:940px;height:400px;" ></table> 
        <br /> 
...
<div id="dialog-modal" title="DETAIL USULAN PENGHAPUSAN">
		...
		<div>
		  <table>
            <tr>
                ...
                <td>:</td>
                <td><input disabled="true" id="no_hapus" name="no_hapus" style="width: 300px; border:0;" /><input type="hidden" id="nomor_bukti" name="nomor_bukti" style="width: 160px;" /></td>
            </tr>
            <tr>
            ...
            <tr>
                <td height="30px">PENGHAPUSAN OPD</td>
                <td>:</td>
    ...
    <fieldset>
        <table align="center">
            <tr>
                <td>
					<a class="easyui-linkbutton" iconCls="icon-cancel" plain="false" onclick="javascript:alasant();">TOLAK USULAN</a>
                    <a class="easyui-linkbutton" iconCls="icon-ok" id="tetap" plain="false" onclick="javascript:simpan();">TETAPKAN USULAN</a> 
...
<div id="dialog-alasan" title="">
    ...
    <fieldset>
        <table align="center">
            <tr>
                <td>
                    <a class="easyui-linkbutton" iconCls="icon-ok" plain="false" onclick="javascript:tolak();">OK</a> 
					<a class="easyui-linkbutton" iconCls="icon-undo" plain="false" onclick="javascript:keluar();">KEMBALI</a>					
				</td>
            </tr>
        </table>   
