...
// -----------------------------------------------------
		...
		//$cRet   .= "<option value='4'>LS Gaji</option>"; 
		//$cRet   .= "<option value='5'>LS PPKD</option>"; 
		//$cRet   .= "<option value='6'>LS Barang & Jasa</option>";
        //$cRet   .= "<option value='7'>LS Bendahara</option>"; 
                               
        $cRet   .= "</select>";        
        return $cRet;
    }

    function combo_jbeban($id='',$script=''){
        $cRet    = '';                        
        $cRet    = "<select name=\"$id\" id=\"$id\" $script >";
        $cRet   .= "<option value=''>Pilih Beban</option>";                 
        $cRet   .= "<option value='1'>UP/GU</option>";
        $cRet   .= "<option value='2'>TU</option>"; 
        $cRet   .= "<option value='3'>LS</option>"; 
        $cRet   .= "<option value='4'>LS Bendahara</option>"; 
        $cRet   .= "</select>";        
        return $cRet;
        }
// -----------------------------------------------------	
...
//------------------------------------wasisw
        function komas($x){
        $hasil="";
        $ambil=array("nol","satu", "dua", "tiga", "empat", "lima", "enam", "tujuh", "delapan", "sembilan");
        $split = explode('.', $x);
        $hasil=$split[1];
        $depan      =substr($hasil,0,1);
        $belakang   =substr($hasil,1,1);
        if ($hasil ==00)
         return " ";
        else
         return " koma ".$ambil[$depan]." ".$ambil[$belakang];
        
    }

    
    function terbilangww ($x) {
        $abil = array("", "satu", "dua", "tiga", "empat", "lima", "enam", "tujuh", "delapan", "sembilan", "sepuluh", "sebelas");
       if ($x < 12)
         return " " . $abil[$x] ;
       elseif ($x < 20)
         return $this->terbilangww($x - 10) . " belas";
       elseif ($x < 100)
         return $this->terbilangww($x / 10) . " puluh" . $this->terbilangww(fmod($x,10));
       elseif ($x < 200)
         return " seratus" . $this->terbilangww($x - 100);
       elseif ($x < 1000)
         return $this->terbilangww($x / 100) . " ratus" . $this->terbilangww(fmod($x,100));
       elseif ($x < 2000)
         return " seribu" . $this->terbilang($x - 1000);
       elseif ($x < 1000000)
         return $this->terbilangww($x / 1000) . " ribu" . $this->terbilangww(fmod($x,1000));
       elseif ($x < 1000000000)
         return $this->terbilangww($x / 1000000) . " juta" . $this->terbilangww(fmod($x,1000000));
       elseif ($x < 1000000000000)
         return $this->terbilangww($x / 1000000000) . " milyar" . $this->terbilangww(fmod($x,1000000000));
       elseif ($x < 1000000000000000)
         return $this->terbilangww($x / 1000000000000) . " trilyun" . $this->terbilangww(fmod($x,1000000000000));
       break;

    }
    
    function gabungs($x){
        $hasil =$this->terbilangww($x);
        $hasil1 = $this->komas($x);
